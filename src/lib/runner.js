...
function getReadmeShowcaseEntries(entries) {
  return entries.filter((entry) => !isMintGeneratedRegistryEntry(entry));
}

function renderReadmeShowcase(targetRoot, entries) {
  const showcaseEntries = getReadmeShowcaseEntries(entries);
  const projectName = path.basename(targetRoot);
  const totalFiles = showcaseEntries.reduce((sum, entry) => sum + entry.files.length, 0);
  const totalVerificationSteps = showcaseEntries.reduce((sum, entry) => sum + entry.verification.length, 0);
  const lines = [
    ...
    "## Statistics",
    "",
    `- Entries: ${showcaseEntries.length}`,
...
function executeWikiMint(targetRoot, flags) {
  ...
  if (mode === "generate" && format !== "readme-showcase") {
    throw new Error(`Unsupported wiki-mint format: ${format}`);
  }
  ...
  const rendered = renderReadmeShowcase(resolvedRoot, entries);
  ...
  const showcaseEntryCount = getReadmeShowcaseEntries(entries).length;
  executeWikiRegister(resolvedRoot, {
    title: `Knowledge Mint: ${format} generated`,
    summary: `Generated ${outputFileName} from ${showcaseEntryCount} Build-Registry entries.`,
